...
import { Check, Calendar, User, ShoppingCart, Shield, Award, Crown } from "lucide-react";
...
export const MajorProjectCard = ({
  ...
  return (
    <div className={`bg-white/95 backdrop-blur-sm rounded-3xl shadow-2xl p-8 md:p-12 border-2 ${
      isPremium 
        ? 'border-gradient-to-r from-amber-400 via-yellow-500 to-gold-600 bg-gradient-to-br from-amber-50/30 to-yellow-50/30 shadow-amber-200/50' 
        : 'border-slate-200 hover:border-blue-300'
    } hover:shadow-3xl transition-all duration-500 hover:scale-[1.02] relative overflow-hidden`}>
      
      {/* Premium Gradient Background */}
      {isPremium && (
        <div className="absolute inset-0 bg-gradient-to-br from-amber-50/40 via-yellow-50/30 to-gold-50/20 rounded-3xl"></div>
      )}
      
      {/* Premium Badge */}
      {isPremium && (
        <div className="absolute top-0 right-0 bg-gradient-to-r from-amber-500 via-yellow-500 to-gold-600 text-white px-6 py-3 text-sm font-bold rounded-bl-2xl shadow-lg z-10">
      ...
      <div className="relative z-10">
        <div className="flex items-center gap-4 mb-6 flex-wrap">
          ...
          {isWithGuide && (
            <span className="bg-gradient-to-r from-amber-600 via-yellow-600 to-gold-700 text-white px-4 py-2 rounded-full text-sm font-bold shadow-lg border border-amber-300">
        ...
        <div className="space-y-4 mb-8">
          <Button 
            ...
            className={`w-full ${
              isWithGuide 
                ? 'bg-gradient-to-r from-amber-600 via-yellow-600 to-gold-700 hover:from-amber-700 hover:via-yellow-700 hover:to-gold-800 text-white shadow-2xl border-2 border-amber-300 hover:border-amber-400' 
          ...
          >
            {isWithGuide && <Crown className="w-5 h-5 mr-2" />}
          ...
          <Button 
            ...
            className={`w-full border-2 ${
              isWithGuide 
                ? 'border-amber-500 text-amber-700 hover:bg-amber-50 hover:border-amber-600 bg-amber-50/50' 
        ...
        <div className={`${
          isWithGuide 
            ? 'bg-gradient-to-br from-amber-50/80 to-yellow-50/80 border-2 border-amber-200' 
